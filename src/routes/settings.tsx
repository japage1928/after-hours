...
const settingsSearchSchema = z.object({
  tab: z.enum(TABS).optional().catch("settings"),
});
...
function SettingsPage() {
  ...
  const tab: SettingsTab = rawTab ?? "settings";
    ...
    <div className="mx-auto flex min-h-dvh max-w-5xl flex-col gap-8 px-4 py-10 md:px-8">
      ...
      <div className="flex flex-col gap-6 md:flex-row md:gap-10">
        ...
        >
          {TABS.map((id) => (
            <Link
              ...
              search={{ tab: id }}
              className={cn(
                "rounded-md px-3 py-2 text-sm capitalize whitespace-nowrap transition-colors",
                tab === id
        ...
        <div className="min-w-0 flex-1">
          {tab === "settings" ? <SettingsTabPanel /> : null}
          {tab === "projects" ? <ProjectsTabPanel /> : null}
          {tab === "profile" ? <ProfileTabPanel /> : null}
          {tab === "account" ? <AccountTabPanel /> : null}
          {tab === "billing" ? <BillingTabPanel /> : null}
        </div>
      </div>
    </div>
  );
...
function SettingsTabPanel() {
  ...
  return (
    <Panel
      title="Settings"
      blurb="Booth defaults that stay on this device."
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        {providers.length === 0 ? (
          ...
          <ul className="flex flex-col gap-1 text-sm text-fg">
            {providers.map((p) => (
              <li key={`${p.providerId}-${p.createdAt}`}>
                {p.providerId}
              </li>
            ))}
          </ul>
...
function BillingTabPanel() {
  ...
  return (
    <Panel
      title="Billing"
      blurb="Plans, usage, and cancel — cancel stays on this tab even when you have no plan."
    >
        ...
        <div className="flex flex-col gap-1 text-sm text-muted">
          ...
          {sub?.cancelAtPeriodEnd && periodLabel ? (
            <p className="text-rec">
              Cancels at period end ({periodLabel}).
            </p>
          ) : periodLabel ? (
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-line bg-surface-2/60 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            variant="rec"
            disabled={!canCancel || Boolean(busy) || Boolean(sub?.cancelAtPeriodEnd)}
